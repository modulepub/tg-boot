...
const handleExpandChange = (row: any, expandedRows: any) => {
	// 记录当前展开的节点ID
	expandedKeys.value = Array.isArray(expandedRows) ? expandedRows.map(item => item.perCode) : []
}

// 是否展开，默认全部展开
const isExpandAll = ref(true)

// 展开所有节点（简化版，因为数据是一次性加载的）
const expandAllNodes = async () => {
	if (!tableRef.value) return
	// 等待DOM更新
	await nextTick()
	// 直接使用表格的展开方法，因为数据是一次性加载的
	state.dataList.forEach(node => {
