...
const uploadSuccess = (response: { fullFilePath: string } | undefined, uploadFile: UploadFile) => {
	if (!response) {
		return
	}
	uploadFile.url = response.fullFilePath

	if (model.value) {
		model.value = model.value + ',' + response.fullFilePath
	} else {
		model.value = response.fullFilePath
	}
}
