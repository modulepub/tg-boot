<template>
	...
	<tg-user-dialog v-if="!disabled && visible" :key="visible" v-model="visible" :multiple @select="userHandle"></tg-user-dialog>
</template>
...
watch(
	...
	async val => {
		if (val) {
			...
			if (idList.length > 0) {
				const { data } = await service.post('/sys/user/nameList', idList)
