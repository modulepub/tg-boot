<template>
	<el-form ref="loginFormRef" :model="loginForm" :rules="loginRules" class="login-form" @keyup.enter="onLogin">
		<el-form-item prop="mobile">
			<el-input v-model="loginForm.phone" :placeholder="$t('app.mobile')">
				<template #prefix>
		...
		<el-form-item prop="code" class="login-code">
			<el-input v-model="loginForm.smsAuthCode" :placeholder="$t('app.captcha')">
				<template #prefix>
					<div class="input-icon">
						<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
// 发送短信验证码
const sendCode = () => {
	if (!mobileRegExp.test(loginForm.phone)) {
