		...
		>
			...
			<el-table-column prop="openType" label="平台类型" header-align="center" align="center" width="180">
				<template #default="scope">
					<el-text v-if="scope.row.openType === 'wechat_work'"><tg-icon icon="icon-workweixin" size="16" /> 企业微信</el-text>
					<el-text v-else-if="scope.row.openType === 'dingtalk'"><tg-icon icon="icon-dingding" size="16" /> 钉钉</el-text>
					<el-text v-else-if="scope.row.openType === 'feishu'"><tg-icon icon="icon-feishu" size="16" /> 飞书</el-text>
					<el-text v-else-if="scope.row.openType === 'wechat_open'"><tg-icon icon="icon-weixin" size="16" /> 微信</el-text>
				</template>
			</el-table-column>
