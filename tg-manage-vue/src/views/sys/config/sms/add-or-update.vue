	...
	<el-drawer v-model="visible" :title="!dataForm.id ? '新增' : '修改'" :size="800">
		<el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="100px" @keyup.enter="submitHandle()">
			<el-form-item label="平台类型" prop="platform">
				<tg-dict-select v-model="dataForm.platform" dict-type="sms_platform" placeholder="平台类型" style="width: 100%"></tg-dict-select>
			</el-form-item>
...
const getSmsPlatform = (id: number) => {
  service.get('/sys/sms/config/' + id).then(res => {
...
const submitHandle = () => {
    ...
    let http
    if (dataForm.id) {
      http = service.put('/sys/sms/config', dataForm)
    } else {
      http =  service.post('/sys/sms/config', dataForm)
    }
    http.then(() => {
			ElMessage.success({
				message: '操作成功',
