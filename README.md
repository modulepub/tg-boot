...
**AI 生成代码前请先读**：[AGENTS.md](AGENTS.md)（实体 `BaseEntity`、双主键、`getByCode` 等硬性约定与检查清单）。

**部署与生产运维**：[OPS.md](OPS.md)（含 **Actuator 禁止对公网暴露**、MQ、架构测试门禁）。
...
└── spring-boot-starter-module
    ...
    │   └── spring-boot-starter-verification（二要素、资产认证、内容合法校验等）
...
#### 与 `StatusCodeEnum` 的关系（勿混淆 YES/NO 字面量）

实现类：`pub.module.common.enums.StatusCodeEnum`（`NO("0")`、`YES("1")`）。

| 场景 | 正确做法 | 错误做法（曾导致前后端状态不一致） |
|------|----------|-----------------------------------|
| 库表 / API JSON | `"0"`、`"1"` | `"YES"`、`"NO"` |
| Java 写入 | `StatusCodeEnum.YES.getCode()` | 字符串 `"YES"` |
| Java 比较 | `StatusCodeEnum.YES.getCode().equals(field)` | `field.equals("YES")` |
| Swagger / 字段注释 | `0=否 1=是` | `YES-是 NO-否` |
| 管理端 / H5 筛选与展示 | `value="0"` / `value="1"` 或 `isStatusYes(code)` | `=== 'YES'`、`el-option value="NO"` |

枚举常量 **名称** 为 `YES`/`NO` 仅用于 Java 代码可读性，**持久化与接口契约永远是 `"0"`/`"1"`**。旧 SQL 注释若写 `YES-是` 属历史笔误，以运行时代码与上表为准。
