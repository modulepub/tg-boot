...
#### **TG** 基于springboot且继承了springboot的设计理念，同时融入了模块化单体的设计理念，便于不同规模系统的团队分工协作，也便于功能解耦。
...
> 概念示例: 比如获取缓存的登陆对象，常见的设计是使用redisTemplate取出字符串再序列化成实体，这样不仅强制依赖redis，还是重复造轮子的做法。
> 最优的做法是从jwt token 解析出username ,再从加了 spring cache 缓存注解的查询方法由框架自动获取缓存和序列化成登陆对象。
> spring cache是一种无关底层缓存产品的设计，底层可以是redis也可以是mem cache、Guava Cache、Ehcache等。spring cache之所以封装了细节只让使用注解，不提供直接操作缓存的接口，是因为spring 认为缓存是业务方法查询效率的辅助，而不该侵入到方法里面，而直接获取redisTemplate的字符串再序列化成实体更是一种不可取的做法，这就违背了spring cache的设计理念，正确的做法是从jwt token 解析出username ,再从服务 UserService getLoginUser（加spring cache 缓存注解，由框架自动获取缓存和序列化）获取。
