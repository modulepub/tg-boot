...
async function proceedGoNext() {
  ...
  try {
    if (!baseForm.cityCode.trim()) {
      uni.showToast({ title: '请先选择常驻城市', icon: 'none' })
      return
    }
    await persistStep1Basic()
    // #ifdef H5
    uni.navigateTo({ url: '/pages/h5/realNameAuthentication' })
    // #endif
    // #ifndef H5
    jump('h5:/pages/h5/realNameAuthentication', '实名认证')
    // #endif
  }
  catch (e) {
    console.error(e)
...
const canGoNext = computed(() => !!baseForm.cityCode.trim())
...
function goPrev() {
  ...
  if (pages.length > 1)
    ...
    uni.navigateTo({ url: '/pages/h5/intake-photo' })
}
...
<template>
  <view class="intake-root">
    <view class="intake-inner">
      <view class="intake-header">
        ...
        <text class="intake-desc">
          信息仅用于推荐展示，我们严守隐私。头像与生活照请在「形象上传」中维护。
        </text>
      </view>

      <view class="intake-card">
          <view class="intake-field" @tap.stop="openCityPicker">
            <text class="intake-field-label">
              常驻城市<text class="intake-req">
                *
              </text>
            </text>
            <view class="intake-field-row">
              <text :class="['intake-field-value', { 'intake-field-placeholder': !baseForm.city }]">
                {{ baseForm.city || '请选择城市' }}
              </text>
              <text class="intake-chevron">
                ›
              </text>
            </view>
          </view>

          <view class="intake-field">
            <text class="intake-field-label">
              职业
            </text>
            <input
              v-model="baseForm.occupation"
              class="intake-field-input"
              :maxlength="64"
          ...
          <view class="intake-switch-card">
            <view class="intake-switch-row">
              <text class="intake-switch-label">
                是否有房
              </text>
              <view
                class="intake-switch-track"
                :class="{ 'intake-switch-track--on': baseForm.hasHouse }"
                @tap.stop="toggleIntakeBool('hasHouse')"
              >
                <view class="intake-switch-thumb" :class="{ 'intake-switch-thumb--on': baseForm.hasHouse }" />
              </view>
            </view>
            <view class="intake-switch-row">
              <text class="intake-switch-label">
              ...
              <view
                class="intake-switch-track"
                :class="{ 'intake-switch-track--on': baseForm.hasCar }"
                @tap.stop="toggleIntakeBool('hasCar')"
              >
                <view class="intake-switch-thumb" :class="{ 'intake-switch-thumb--on': baseForm.hasCar }" />
              </view>
            </view>
            <view class="intake-switch-row">
              <text class="intake-switch-label">
              ...
              <view
                class="intake-switch-track"
                :class="{ 'intake-switch-track--on': baseForm.hasDisability }"
                @tap.stop="toggleIntakeBool('hasDisability')"
              >
                <view class="intake-switch-thumb" :class="{ 'intake-switch-thumb--on': baseForm.hasDisability }" />
              </view>
            </view>
            <view class="intake-switch-row intake-switch-row--last">
              <text class="intake-switch-label">
                是否二婚
              </text>
              <view
                class="intake-switch-track"
                :class="{ 'intake-switch-track--on': baseForm.isRemarriage }"
                @tap.stop="toggleIntakeBool('isRemarriage')"
              >
                <view class="intake-switch-thumb" :class="{ 'intake-switch-thumb--on': baseForm.isRemarriage }" />
              </view>
            </view>
          </view>

          <view class="intake-field intake-field--stack">
            <text class="intake-field-label">
              说说
            </text>
            <textarea
              v-model="baseForm.say"
    ...
    >
      <view class="intake-footer-btns">
        <view
          class="intake-btn intake-btn--secondary"
        ...
        <view
          class="intake-btn intake-btn--primary"
          :class="{ 'intake-btn--disabled': stepSaving }"
          role="button"
          @tap.stop="goNext"
        >
