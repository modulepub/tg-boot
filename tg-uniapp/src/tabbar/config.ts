...
/** H5 官网形态：无底部 Tab，通过首页右上角菜单导航 */
export const selectedTabbarStrategy = TABBAR_STRATEGY_MAP.NO_TABBAR

// TODO: 2/3. 使用 NATIVE_TABBAR 时，请自行准备 81×81 PNG 等图标并改回 iconPath（当前项目仅小程序 + 自定义 TabBar，不维护 static/tabbar 位图）
...
/**
 * 始终写入 pages.json。
 * NO_TABBAR 时若不配置 tabBar，H5 运行时在 useShowTabBar 读取 tabBar.height 会报错；
 * 视觉上通过 App 层 uni.hideTabBar + App.ku 不渲染自定义 Tab 实现无底栏。
 */
