...
public class WebQueryUtil {
    ...
    public static <T> QueryWrapper<T> buildQuery(T searchObj) {
        ...
        for (Field field : ReflectUtil.getFields(searchObj.getClass())) {
            ...
            if (fieldValue != null) {
                ...
                } else if (fieldValue instanceof String && StrUtil.isNotEmpty(fieldValue.toString())) {
                    queryWrapper.eq(StrUtil.toUnderlineCase(fieldName), fieldValue.toString());
                }
                else if (fieldValue instanceof BaseEnum baseEnum) {
                    queryWrapper.eq(StrUtil.toUnderlineCase(fieldName), baseEnum.getCode());
                }
    ...
    public static void setSelect(QueryWrapper<?> queryWrapper, Class<?> tClass) {
        String[] fieldNames = Arrays.stream(ReflectUtil.getFields(tClass)).map(Field::getName).map(StrUtil::toUnderlineCase).toArray(String[]::new);
        queryWrapper.select(fieldNames);
    }
