...
@Slf4j
@Service
public class CustomerPromotionTaskServiceImpl extends ServiceImpl<CustomerPromotionTaskMapper, CustomerPromotionTask> implements CustomerPromotionTaskService {

    String bizCode = "promotionTaskCode";

    public void setDefaultValue(CustomerPromotionTask entity) {
    ...
    @Override
    public CustomerPromotionTask getByCode(String code) {
        return this.getBaseMapper().selectOne(new QueryWrapper<CustomerPromotionTask>().eq(StrUtil.toUnderlineCase(bizCode), code),false);
    }

    @Override
    @Transactional
    public boolean save(CustomerPromotionTask entity) {
    ...
    @Override
    @Transactional
    public boolean saveBatch(Collection<CustomerPromotionTask> entityList) {
        for ( CustomerPromotionTask entity : entityList) {
            this.save(entity);
        }
        return true;
    }
    ...
    public boolean removeById(Serializable id) {
        CustomerPromotionTask entity = this.getBaseMapper().selectById(id);
    ...
    @Override
    @Transactional
    public boolean updateById(CustomerPromotionTask entity) {
    ...
    @Override
    public CustomerPromotionTask getById(Serializable id) {
        CustomerPromotionTask entity = this.getBaseMapper().selectById(id);
        Assert.notNull(entity, "CustomerTelemarketerRelation 不存在");
        this.setDefaultValue(entity);
        return entity;
    ...
    @Override
    public CustomerPromotionTask getOne(Wrapper<CustomerPromotionTask> queryWrapper,
                                        boolean throwEx) {
        CustomerPromotionTask entity = null;
        List<CustomerPromotionTask> list = this.list(queryWrapper);
        if (!list.isEmpty()) {
            entity = list.get(0);
            this.setDefaultValue(entity);
        }
