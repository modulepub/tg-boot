...
@Slf4j
@Service
public class CustomerContactRecordServiceImpl extends ServiceImpl<CustomerContactRecordMapper, CustomerContactRecord> implements CustomerContactRecordService {

    String bizCode = "contactRecordCode";

    public void setDefaultValue(CustomerContactRecord entity) {
    ...
    @Override
    public CustomerContactRecord getByCode(String code) {
        return this.getBaseMapper().selectOne(new QueryWrapper<CustomerContactRecord>().eq(StrUtil.toUnderlineCase(bizCode), code),false);
    }

    @Override
    @Transactional
    public boolean save(CustomerContactRecord entity) {
    ...
    @Override
    @Transactional
    public boolean saveBatch(Collection<CustomerContactRecord> entityList) {
        for ( CustomerContactRecord entity : entityList) {
            this.save(entity);
        }
        return true;
    }
    ...
    public boolean removeById(Serializable id) {
        CustomerContactRecord entity = this.getBaseMapper().selectById(id);
        Assert.notNull(entity, "CustomerContactRecord 不存在");
    ...
    @Override
    @Transactional
    public boolean updateById(CustomerContactRecord entity) {
    ...
    @Override
    public CustomerContactRecord getById(Serializable id) {
        CustomerContactRecord entity = this.getBaseMapper().selectById(id);
        Assert.notNull(entity, "CustomerContactRecord 不存在");
        this.setDefaultValue(entity);
        return entity;
    ...
    @Override
    public CustomerContactRecord getOne(Wrapper<CustomerContactRecord> queryWrapper,
                                boolean throwEx) {
        CustomerContactRecord entity = null;
        List<CustomerContactRecord> list = this.list(queryWrapper);
        if (!list.isEmpty()) {
            entity = list.get(0);
            this.setDefaultValue(entity);
        }
