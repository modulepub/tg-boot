...
public class DtPreferenceDisplayService {
    ...
    public void enrichPeerDisplay(IPage<DtPreference> page, boolean fillTarget) {
        ...
        Map<String, DtCustomerDTO> cusMap = apiDtCustomerService.listByCusCodes(peerCodes).stream()
                .filter(c -> StrUtil.isNotBlank(c.getCusCode()))
                .collect(Collectors.toMap(DtCustomerDTO::getCusCode, Function.identity(), (a, b) -> a));
        for (DtPreference row : page.getRecords()) {
            String peerCode = fillTarget ? row.getPreferenceTargetCusCode() : row.getPreferenceCusCode();
            DtCustomerDTO cus = cusMap.get(peerCode);
            if (fillTarget) {
    ...
    public static void fillTargetFromCustomer(DtPreference pref, DtCustomerDTO cus) {
    ...
    public static void fillCusFromCustomer(DtPreference pref, DtCustomerDTO cus) {
        if (pref == null || cus == null) {
            return;
        }
    ...
    public static String resolveDisplayName(DtCustomerDTO cus) {
        if (cus == null) {
            return null;
        }
    ...
    public static Integer toIntegerAge(Long age) {
        if (age == null) {
            return null;
        }
        return age.intValue();
