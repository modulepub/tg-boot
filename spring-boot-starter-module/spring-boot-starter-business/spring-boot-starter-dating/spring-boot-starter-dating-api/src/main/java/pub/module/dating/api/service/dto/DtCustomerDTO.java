...
public class DtCustomerDTO extends BaseEntity {
    /**
     * 客户编号
     */
    ...
    private CusFollowUpReminderTypeCodeEnum cusFollowUpReminderTypeCode;
        ...
        @Schema(description = "添加好友次数权益（历史累计字段）")
        private Long cusAddFriendRightValue;

        @Schema(description = "推荐次数权益（历史累计字段）")
        private Long cusRecommendRightValue;

        @Schema(description = "牵线次数权益（历史累计字段）")
        ...
        @TableField(exist = false)
        @Schema(description = "当日已用添加好友次数")
        private Long cusAddFriendDayUsed;

        @TableField(exist = false)
        @Schema(description = "当日已用推荐次数")
        private Long cusRecommendDayUsed;

        @TableField(exist = false)
        @Schema(description = "当日已用牵线申请次数")
        private Long cusMatchDayUsed;

        @TableField(exist = false)
        @Schema(description = "资料完整度（0-100），按用户端可编辑字段非空占比计算")
        private Integer cusProfileCompletenessRate;
