...
public class CusTradeGoodsController {
    ...
    /**
     * 移动端不查询隐藏商品（tdGdHiddenStatusCode = 1）。兼容历史空值。
     */
    private void excludeHidden(QueryWrapper<TdGoods> queryWrapper) {
        queryWrapper.lambda().and(w -> w
                .ne(TdGoods::getTdGdHiddenStatusCode, StatusCodeEnum.YES.getCode())
                .or().isNull(TdGoods::getTdGdHiddenStatusCode));
    }
