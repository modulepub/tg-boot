...
public class TdOrderController{
	...
	@Operation(summary="td_order-通过id删除")
	@DeleteMapping(value = "/delete")
	public Result<String> delete(@RequestParam(name="id") String id) {
		tdOrderService.removeById(id);
		return Result.ok("删除成功!");
	}
	
	@Operation(summary="td_order-批量删除")
	@DeleteMapping(value = "/deleteBatch")
	public Result<String> deleteBatch(@RequestParam(name="ids") String ids) {
		this.tdOrderService.removeByIds(Arrays.asList(ids.split(",")));
		return Result.ok("批量删除成功!");
	}
	
	@Operation(summary="td_order-通过id查询")
