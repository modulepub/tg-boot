...
public class MgtTdGoodsController {
	...
	@Operation(summary = "管理端-td_goods-上下架")
	@PostMapping(value = "/updateShelf")
	public Result<String> updateShelf(@RequestParam(name = "id") String id,
	                                  @RequestParam(name = "tdGdEnabledCode") String tdGdEnabledCode) {
		if (!"0".equals(tdGdEnabledCode) && !"1".equals(tdGdEnabledCode)) {
			return Result.error("上下架状态无效");
		}
		boolean updated = tdGoodsService.lambdaUpdate()
				.set(TdGoods::getTdGdEnabledCode, tdGdEnabledCode)
				.eq(TdGoods::getId, id)
				.update();
		if (!updated) {
			return Result.error("商品不存在或更新失败");
		}
		return Result.ok("1".equals(tdGdEnabledCode) ? "已上架" : "已下架");
	}
