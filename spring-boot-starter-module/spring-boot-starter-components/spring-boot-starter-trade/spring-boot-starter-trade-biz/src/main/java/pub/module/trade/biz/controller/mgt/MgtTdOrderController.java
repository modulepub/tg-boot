...
public class MgtTdOrderController {
	...
	public Result<IPage<TdOrder>> queryPageList(TdOrder tdOrder,
		...
		QueryWrapper<TdOrder> queryWrapper = buildOrderQueryWrapper(tdOrder);
		Page<TdOrder> page = new Page<>(pageNo, pageSize);
		IPage<TdOrder> pageList = tdOrderService.page(page, queryWrapper);
		return Result.ok(pageList);
	}

	@Operation(summary = "管理端-td_order-金额统计")
	@GetMapping(value = "/statistics")
	public Result<TdOrderStatisticsDTO> statistics() {
		return Result.ok(tdOrderService.getStatistics());
	}
	...
	private QueryWrapper<TdOrder> buildOrderQueryWrapper(TdOrder tdOrder) {
		QueryWrapper<TdOrder> queryWrapper = WebQueryUtil.buildQuery(tdOrder);
		if (tdOrder.getCreateDateRangeArray() != null && tdOrder.getCreateDateRangeArray().length == 2) {
			queryWrapper.ge("DATE(create_time)", tdOrder.getCreateDateRangeArray()[0]);
			queryWrapper.le("DATE(create_time)", tdOrder.getCreateDateRangeArray()[1]);
		}
		return queryWrapper;
	}
