...
public class MinioUploadService {
    ...
    public MinioClient getMinioClient() {
        ...
        buketName = jsonObject.getJSONObject("minio").getStr("bucketName");
        return MinioClient.builder().endpoint(jsonObject.getJSONObject("minio").getStr("minioUrl")).credentials(jsonObject.getStr("minioName"), jsonObject.getStr("minioPass")).build();
    }
    ...
    public int uploadByFragment(File file, String md5, Integer sliceIndex, Integer totalPieces,String filePath) {
        ...
        // 调用分片上传的逻辑
        int index = uploadFileByFragment(file, sliceIndex, totalPieces, md5, buketName);
        if (index == -1) {
            mergeFragmentFile(totalPieces, md5,  buketName,filePath);
        }
        return index;
    }
    ...
    public int uploadFileByFragment(File file, Integer sliceIndex, Integer totalPieces, String md5, String bucketName) {
        Iterable<Result<Item>> results = this.getMinioClient().listObjects(ListObjectsArgs.builder().bucket(bucketName).prefix(md5.concat("/")).build());
        ...
        this.getMinioClient().putObject(PutObjectArgs.builder().bucket(bucketName)
    ...
    public void mergeFragmentFile(Integer totalPieces, String md5, String bucketName,String filePath) throws Exception {
        ...
        this.getMinioClient().composeObject(ComposeObjectArgs.builder().bucket(bucketName).object(filePath).sources(sourceObjectList).build());
        ...
        Iterable<Result<DeleteError>> results = this.getMinioClient().removeObjects(RemoveObjectsArgs.builder().bucket(bucketName).objects(delObjects).build());
