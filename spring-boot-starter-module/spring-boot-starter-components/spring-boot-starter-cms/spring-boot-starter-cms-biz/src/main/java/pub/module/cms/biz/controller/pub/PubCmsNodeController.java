...
public class PubCmsNodeController {
    ...
    @Resource
    private SpiCmsNodeReadService spiCmsNodeReadService;
    ...
    public Result<IPage<CmsNodeDTO>> queryPageList(CmsNode cmsNode,
        ...
        queryWrapper.lambda().eq(CmsNode::getNodePublishStatusCode, StatusCodeEnum.YES);
    ...
    @Operation(summary = "公开-CMS-开始阅读（生成会话编码并累计阅读数）")
    @PostMapping(value = "/readStart")
    public Result<CmsNodeReadStartVO> readStart(@RequestBody CmsNodeReadStartReq req) {
        return Result.ok(spiCmsNodeReadService.startRead(req));
    }

    @Operation(summary = "公开-CMS-上报阅读进度")
    @PostMapping(value = "/readProgress")
    public Result<String> readProgress(@RequestBody CmsNodeReadProgressReq req) {
        spiCmsNodeReadService.reportProgress(req);
        return Result.ok("ok");
    }
