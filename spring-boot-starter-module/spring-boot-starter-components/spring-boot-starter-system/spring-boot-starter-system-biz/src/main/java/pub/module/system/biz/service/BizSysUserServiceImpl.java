...
public class BizSysUserServiceImpl implements BizSysUserService {
    ...
    public LoginDTO loginByCode(String userCode) {
        ...
        // 进行用户名密码认证
        List<String> roleCodes = bizSysUserOrganizationRoleService.getRolesByOrgCodeAndUserCode(sysUser.getOrgCode(),userCode);
        roleCodes.add("-");
        List<String> perCodes = bizSysRolePermissionService.getPermissionsByRoles(roleCodes);
        perCodes.add("-");
        String token = JwtTokenProvider.generateToken(sysUserName, sysUser.getUserPassword(), perCodes);
    ...
    public void logout(String userCode) {
        UserDTO sysUser = SpringUtil.getBean(BizSysUserService.class).getUserByUserCode(userCode);
        Assert.notNull(sysUser, "sysUser is null");
