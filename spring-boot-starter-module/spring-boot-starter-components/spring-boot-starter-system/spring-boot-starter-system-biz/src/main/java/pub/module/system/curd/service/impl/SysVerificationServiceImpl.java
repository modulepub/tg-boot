...
import pub.module.system.curd.mapper.SysVerificationMapper;
...
@Slf4j
@Service
public class SysVerificationServiceImpl extends ServiceImpl<SysVerificationMapper, SysVerification> implements SysVerificationService {

    String bizCode = "verificationCode";

    public void setDefaultValue(SysVerification entity) {
    ...
    @Override
    public SysVerification getByCode(String code) {
        return this.getBaseMapper().selectOne(new QueryWrapper<SysVerification>().eq(StrUtil.toUnderlineCase(bizCode), code),false);
    }

    @Override
    @Transactional
    public boolean save(SysVerification entity) {
    ...
    @Override
    @Transactional
    public boolean saveBatch(Collection<SysVerification> entityList) {
        for ( SysVerification entity : entityList) {
            this.save(entity);
        }
        return true;
    }
    ...
    public boolean removeById(Serializable id) {
        SysVerification entity = this.getBaseMapper().selectById(id);
        Assert.notNull(entity, "SysVerification 不存在");
    ...
    @Override
    @Transactional
    public boolean updateById(SysVerification entity) {
    ...
    @Override
    public SysVerification getById(Serializable id) {
        SysVerification entity = this.getBaseMapper().selectById(id);
        Assert.notNull(entity, "SysVerification 不存在");
        this.setDefaultValue(entity);
        return entity;
    ...
    @Override
    public SysVerification getOne(Wrapper<SysVerification> queryWrapper,
                                boolean throwEx) {
        SysVerification entity = null;
        List<SysVerification> list = this.list(queryWrapper);
        if (!list.isEmpty()) {
            entity = list.get(0);
            this.setDefaultValue(entity);
        }
