...
public class CusPermissionController {
    ...
    public Result<PermissionDTO> listAll(@RequestParam(name="code") String perCode) {
        ...
        List<PermissionDTO> permissionDTOList = apiSysPermissionService.getPermissionsByUserCode(userDTO.getUserCode()).stream()
                .filter(permissionDTO -> PerTypeCodeEnum.MENU.equals(permissionDTO.getPerTypeCode()))
                .toList();
