...
public class PubLoginController {
    ...
    @Resource
    ApiWxMaSessionService apiWxMaSessionService;
    ...
    @Data
    @Schema(description = "免鉴权-微信登录VO")
    public static class LoginByWxVO {
    ...
    @Operation(summary = "公开-免鉴权-微信登录接口")
    @PostMapping("/wxLogin")
    public Result<SysUserTokenVO> wxLogin(@RequestBody LoginByWxVO loginByWx) {
        // 与手机号登录功能完全一致，唯一区别是登录方式：用微信 code 换取 openId 作为登录标识
        org.springframework.util.Assert.isTrue(StrUtil.isNotBlank(loginByWx.getAppId()), "appId 不能为空");
        org.springframework.util.Assert.isTrue(StrUtil.isNotBlank(loginByWx.getCode()), "code 不能为空");
        String openId = apiWxMaSessionService.getOpenIdByCode(loginByWx.getAppId().trim(), loginByWx.getCode().trim());
        UserDTO result = apiSysUserService.registerByOpenId(openId, loginByWx.getUserReferenceUserCode());
        ApiSysUserService.LoginDTO loginDTO = apiSysUserService.loginByCode(result.getUserCode());
        SysUserTokenVO token = new SysUserTokenVO(loginDTO.getAccessToken(), loginDTO.getAccessToken(), loginDTO.getExpireTime(), loginDTO.getExpireTime());
        return Result.ok(token);
    }
