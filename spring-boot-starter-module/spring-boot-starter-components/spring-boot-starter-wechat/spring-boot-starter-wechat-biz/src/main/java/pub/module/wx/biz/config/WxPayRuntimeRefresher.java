...
/**
 * 将库表 wx_pay_config 中启用的记录加载为 WxPayService 多商户配置。
 */
@Slf4j
@Component
@ConditionalOnClass(WxPayService.class)
public class WxPayRuntimeRefresher {
    ...
    public synchronized void refreshFromDatabase() {
        List<WxPayConfig> list = wxPayConfigService.lambdaQuery()
                .eq(WxPayConfig::getWxPayConfigEnabledCode, ENABLED_CODE)
                .orderByAsc(WxPayConfig::getSeqNo)
                .orderByAsc(WxPayConfig::getWxPayConfigCode)
                .list();
        if (list.isEmpty()) {
            log.warn("wx_pay_config 无启用配置（wx_pay_config_enabled_code={}），WxPayService 将清空多商户配置", ENABLED_CODE);
            wxPayService.setMultiConfig(Collections.emptyMap());
            return;
        }
        Map<String, com.github.binarywang.wxpay.config.WxPayConfig> map = new LinkedHashMap<>();
        for (WxPayConfig row : list) {
            ...
            com.github.binarywang.wxpay.config.WxPayConfig cfg = new com.github.binarywang.wxpay.config.WxPayConfig();
        ...
        log.info("已从 wx_pay_config 加载 {} 条微信支付配置到 WxPayService", map.size());
    }

    /**
     * 使用 PEM 正文配置 APIv3 私钥与证书。
     */
    private static void configureKeyMaterial(com.github.binarywang.wxpay.config.WxPayConfig cfg,
                                             String privateKeyPem,
                                             String privateCertPem) {
