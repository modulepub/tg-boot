...
public class WxController {
    ...
    @Resource
    WxMpRuntimeRefresher wxMpRuntimeRefresher;
    ...
    public Result<String> getWxaCodeUnlimit(
            @RequestBody WxaCodeUnlimitRequest request,
            @RequestParam(value = "width", required = false) Integer widthQuery) {
        try {
            Integer width = request.getWidth() != null ? request.getWidth() : widthQuery;
    ...
    @Operation(summary = "公开-生成小程序码（高清下载，固定 1280px）")
    @PostMapping("/getWxaCodeUnlimitHd")
    public Result<String> getWxaCodeUnlimitHd(@RequestBody WxaCodeUnlimitRequest request) {
        try {
            byte[] png = WxUtil.getWxaCodeUnlimit(
                    request.getAppId(),
                    request.getPage(),
            ...
            log.warn("生成高清小程序码失败: {}", e.getMessage());
            return Result.error(e.getError().getErrorMsg());
        } catch (IllegalArgumentException e) {
            return Result.error(e.getMessage());
        }
    ...
    /**
     * 微信公众号消息/事件回调（支持明文与安全模式 AES 加解密）。
     */
    ...
    public String post(
        ...
        log.info("收到来自微信服务器的请求 appId={} encType={} openId={} bodyLen={}",
                appid, encType, openid, requestBody == null ? 0 : requestBody.length());
        wxMpRuntimeRefresher.ensureAppLoaded(appid);
        ...
        boolean aesMode = "aes".equalsIgnoreCase(encType);
        WxMpXmlMessage inMessage = aesMode
                ? WxMpXmlMessage.fromEncryptedXml(requestBody, wxMpService.getWxMpConfigStorage(),
