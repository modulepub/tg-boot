...
import pub.module.log.curd.mapper.LogMapper;
...
public class LogServiceImpl extends ServiceImpl<LogMapper, Log> implements LogService {
    ...
    public void setDefaultValue(Log entity) {
    ...
    public Log getByCode(String code) {
        return this.getBaseMapper().selectOne(new QueryWrapper<Log>().eq(StrUtil.toUnderlineCase(bizCode), code),false);
    }
    ...
    @Override
    @Transactional
    public boolean saveBatch(Collection<Log> entityList) {
    ...
    public boolean removeById(Serializable id) {
        Log entity = this.getBaseMapper().selectById(id);
    ...
    @Override
    public Log getOne(Wrapper<Log> queryWrapper,
                                boolean throwEx) {
        Log entity = null;
        List<Log> list = this.list(queryWrapper);
        if (!list.isEmpty()) {
            entity = list.get(0);
            this.setDefaultValue(entity);
        }
